...
public class GoogleAuthenticatorIntegrationController {
    ...
    public static final String ISSUER = "jumpServer";
    ...
    @SneakyThrows
    @GetMapping("/qr_code")
    public void qrCode(HttpServletResponse response) throws UnsupportedEncodingException {
        // 获取用户名称(从数据库或者缓存),可以是登录名,邮箱,手机(不参与运算,只是为了与其他服务作区分)
        String account = "testMfa";
        // 生成密钥,并保存到数据库
